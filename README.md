...
Las tareas se seleccionan desde las Issues habilitadas en GitHub Project.
Cada integrante se asigna la Issue que va a desarrollar.
Antes de comenzar una nueva tarea se actualiza la rama main local.
Cada Issue se desarrolla en una rama independiente creada a partir de main.
Los cambios se registran mediante commits y se publican en la rama correspondiente.
La integración a main se realiza exclusivamente mediante Pull Request.
La rama main se encuentra configurada para impedir el merge de un Pull Request hasta contar con al menos una aprobación de revisión por parte de otro integrante del equipo. Esta regla aplica también cuando el autor del PR es quien posee permisos para realizar el merge.
Una vez aprobado y mergeado el PR, la Issue asociada se considera finalizada.
...
popularidad global;
popularidad por season;
popularidad por Location;
popularidad por interacción season + Location.
...
Auditoría y logs para usuarios con acceso administrativo.
Indexación individual
